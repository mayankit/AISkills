...
See the `agentic-loops` skill (Loop Stage Vocabulary).
...
1. **Ephemeral session notes** — per-session scratch (see `session-control`). Untrusted,
...
2. **Durable local overlays** — `<workspace-root>/.agentic-loops/learnings/<project>.md`,
   ...
   `build-discipline` writes the convention cache at
...
- **Validated on 2+ independent occasions** — different task, file, or session. The same
  ...
  (see `doubt-driven-development`): "I did it and it worked" is how an overfit n=1 poisons
...
`capability-creation`: propose a new skill or agent to the user, and on consent build it with
