...
When you write code applying a pattern, load `build-discipline` (Phase 0 →
...
4. **Record it now** — the ADR is the artifact; a decision that isn't written down will be
   re-litigated. High-stakes decisions get a doubt pass (`doubt-driven-development`) on its
