...
stop:review-clean. Fixing the diff opens a Build graph via `build-discipline` — never patch-and-post
from inside the review.

**Boundary:** for security/threat review, use `code-review`;
pair with `doubt-driven-development` for high-stakes verification.
...
Use `code-review`'s severity ladder: a confirmed injection, auth bypass, or secret in the
...
(`doubt-driven-development`: "I verified because...", not "it should be fine").
