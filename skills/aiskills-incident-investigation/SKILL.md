...
investigation track). See the `aiskills-agentic-loops` skill (Loop Stage Vocabulary).
...
- Mitigation is a production-affecting action — it crosses the Human Gate in `aiskills-agentic-loops`:
...
### Step 5: Verify
Evidence, not vibes — apply `aiskills-doubt-driven-development` to your own theory.
...
- Feed a regression test back through `aiskills-build-discipline` so the bug class can't ship again.
- Add or tune an alarm via `aiskills-observability` — no recurrence is detected before customers report
