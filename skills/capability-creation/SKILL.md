...
runs the full `build-discipline` for the artifact (via `build-discipline`) · stop:capability-shipped-or-declined.

**Loop subgraph** (grammar in `agentic-loops`, activation in `brain/loop-contract.md` — neither
restated here): **User gate** for the artifact (via `build-discipline`) · stop:capability-shipped-or-declined.

## Overview

Overlay notes (`continuous-learning`) are the right home for *lessons*. But when the same
...
- **Recurring** — the same gap appeared on 2+ independent occasions (different task or session)
  (see `continuous-learning` uses for promotion). One occurrence is a note, not a skill.
...
- **Not already covered** — `skill-routing` has no row that matches Primary or extends existing
...
package. That's `continuous-learning`'s promotion gate applied to a whole shared repository later —
that's `continuous-learning`'s promotion gate applied to a whole capability.

## Step 4: BUILD — the new capability gets the full discipline

A capability is a non-executable artifact; `build-discipline`'s TDD for
...
2. **GREEN — author the minimal capability.** Frontmatter (`name:` matching the directory,
   ...
   the grammar which is the ONE owner (`agentic-loops`), a new capability NEVER restates
   that defers explicitly to `brain/loop-contract.md` and names its own owner.
3. **ROUTING — Shared:** add the `skill-routing` row (the new intent -> skill mapping) so the
...
- **Unrouted capabilities** — created but unreachable; if `skill-routing` (or the local routing
