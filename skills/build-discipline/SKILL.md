...
**Loop subgraph** (grammar in `agentic-loops`): L0 Convention-Model → L1 (if files unknown) →
...
2. **Announce each phase.** With `agentic-loops` loaded, this is a status line per phase
...
4. **Review history** — what do reviewers repeatedly flag on merged PRs touching this area
   ...
   `continuous-learning`) so the next session doesn't re-derive them from scratch.
...
Run these checks in order; fix anything that fails before proceeding. When `agentic-loops` is
...
7. **Doubt pass (always)** — proportional to stakes. State the 1-3 load-bearing claims the
   ...
   `doubt-driven-development` cycle on every load-bearing claim.
