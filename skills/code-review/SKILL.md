...
- **A review writes no code.** Do NOT load `build-discipline` for it; if the review leads to
...
2. **Security** — input validation, injection, authorization on every operation, secrets,
   unsafe deserialization (load `security-review` as the lens for a security-sensitive diff).
...
("would the linter or my teammate flag this?"). This is `build-discipline` VERIFY step 6; it is
