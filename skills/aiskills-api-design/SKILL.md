...
the contract in code, load `aiskills-build-discipline` for the how; this skill tells you *what* the
contract is.

**Loop subgraph** (grammar in `aiskills-agentic-loops`, activation in `brain/loop-contract.md` — neither
restated here): **Fan-Out orchestration** — the L3 dispatch/join mechanics live in `aiskills-design`
· stop:contract-agreed. Record expensive-to-reverse contract choices as ADRs (`aiskills-design`, Part 2).
