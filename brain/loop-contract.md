...
- **`agentic-loops`** — the loop taxonomy (L0-L4), status-line format, plan tree, fan-out
  mechanics, stop conditions. Load it for any multi-step task.
- **`build-discipline`** — the delivery SPINE (Phase 0 → RED → GREEN → REFACTOR → VERIFY).
  Load it whenever the row below says Build.

Supporting skills load at their stage, routed by `skill-routing`'s detailed index — skill
selection + ADRs during shaping, the `doubt-driven-development` splice at every hard-to-reverse act.
...
```bash
_LOOP_SH=$(find ~/.claude/skills -maxdepth 6 -path "*agentic-loops/scripts/loop-status.sh" 2>/dev/null | head -1)
...
file — source, tests, prompts, configs, schemas — it is a Build task and `build-discipline`
loads. No exceptions, except the two `build-discipline` documents explicitly on its own terms
...
|---|---|---|
| **Build / bug / refactor** (any behavior-affecting edit) | L0 Convention-Model → L1 (if files unknown) → L2 [RED-GREEN-REFACTOR-VERIFY] | `build-discipline` + `agentic-loops` (grammar) |
| **Code-review** (evaluate a diff — no fixing) | L1 Context (skim tickets, prior comments, diff) → mini-L3 (check dimensions, read-only) → CLOSE = findings report | `agentic-loops` only — a review writes NO code, NOT `build-discipline` |
| **Review → fix transition** | CLOSE the review graph, then open a Build graph — never patch-and-post from inside a review | add `build-discipline` at the transition |
| **Incident / investigation** | Investigate graph: triage → scope → mitigate first → hypothesize → verify → guard | `incident-investigation` |
| **Design** (ORIENT/DECIDE-heavy) | L1 Context → design (ORIENT/DECIDE) → decision recorded as ADR | `design` — no build gate unless code follows |
| **Hard-to-reverse act** (security, data, payments, prod-rollback) | a `[DOUBT]` splice inside whichever graph is already open | `doubt-driven-development` |
| **Routing / quick QA** (single lookup, no state change) | Single pass — plan tree and ledger optional | `skill-routing` if the right skill is unclear |
...
ONE place — the `agentic-loops` skill. Every other skill declares a one-line `Loop subgraph`
...
## Non-negotiable rules (the what — formats live in `agentic-loops`)
...
1. **Plan before the first tool call.** Any task expected to take more than one or two tool
   ...
   in `agentic-loops` — never redefined per skill.
2. **Signal position, don't just narrate it.** Every loop open, close, and DONE gets one status
   line, written through the ledger script (`agentic-loops`'s `loop-status.sh`) when a shell
...
4. **Two strikes, then change approach.** The same fix attempted twice without success is not
   ...
   diagnosis or strategy, not a variation on the same guess (see `agentic-loops`'s L2
   `strike <s>/2` tracking).
...
7. **A build task never ships without its gate passing for real.** `build-discipline`'s coding
