...
Before your first status line, load `agentic-loops` (the loop grammar) and
`incident-investigation` (the process you execute). If your host has no skill loader, read
...
- A dev-time build, test, or local runtime failure is not an incident — hand it to
  `debugging-recovery` and close this graph.
- Missing or inadequate telemetry discovered mid-investigation routes to `observability` —
...
- The fix itself is a graph switch: CLOSE the Investigate track, then OPEN a Build graph via
  `build-discipline`. Never patch from inside the investigation.
- Every hypothesis is verified with evidence before `doubt-driven-development` timeline
...
through `build-discipline`, or an alarm added via `observability`. Slow detection is itself a
