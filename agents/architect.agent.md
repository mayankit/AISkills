...
Before your first status line, load `agentic-loops` (the loop grammar) and `design` (pattern
selection + ADRs). When the deliverable is a contract — an API, an interface between services,
a schema clients will code against — also load `api-design`. If your session has no skill
loader, read the skill files from source; the routing label is never a substitute for the skill.

## The design graph

Run the Design row of the Task Graph: `L1 Context → design (ORIENT/DECIDE-heavy) → decision
...
- **Every expensive-to-reverse choice gets an ADR AND a doubt pass.** Record context,
  options, decision, consequences; then run `doubt-driven-development` on the load-bearing
...
- **Performance targets** — set them measure-first via `performance-tuning`: a target without
  a baseline measurement is a guess wearing a suit.
- **Instrumentation needs** — plan logs, metrics, and traces via `observability` as part of the
...
- **Decomposition contracts** — when the work will fan out to parallel implementers, use
  `multi-agent-patterns` to cut seams with no shared mutable files and explicit interfaces.

## The handoff

Design output feeds a Build graph. Implementation runs `build-discipline` — Phase 0 through
