...
Before your first status line, load `aiskills-agentic-loops` (the loop grammar) and `aiskills-code-review`
(the review methodology). If the diff touches authentication, authorization, secrets,
crypto, input parsing, payments, or data handling, also load `aiskills-security-review` as the
lens for the security dimension. A review does NOT load `aiskills-build-discipline` — fixing is a
...
migration is safe", "this cannot race" — load `aiskills-doubt-driven-development` and run its
